...
#$ -b y -l mem_free=32G -P jravel-lab -q threaded.q -pe thread 16 -N RAxML_MMOTH -j y -o /local/scratch/mfrance/logs/ -e /local/scratch/mfrance/logs/

export LD_LIBRARY_PATH=/usr/local/packages/gcc-9.3.0/lib64:$LD_LIBRARY_PATH

cd /local/scratch/mfrance/EZAK/03_phylogeny
...
/usr/local/packages/raxml-ng-1.0.1mpi/bin/raxml-ng-mpi --all --msa /local/scratch/mfrance/EZAK/03_phylogeny/EZAK_core.phy -msa-format PHYLIP --data-type AA -prefix EZAK --seed 2 --model /local/scratch/mfrance/EZAK/03_phylogeny/EZAK_partitions.txt --tree pars{10},rand{10} --thread 16 --workers 8 --bs-trees autoMRE
